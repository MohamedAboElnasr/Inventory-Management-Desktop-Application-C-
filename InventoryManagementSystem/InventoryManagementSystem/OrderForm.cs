    ...
    public partial class OrderForm : Form
    {
        ...
        public void LoadOrder()
        {
            try {
                ...
                dgvOrder.Rows.Clear();
                con.Open();
                cm = new SqlCommand("SELECT orderid, odate, O.pid, P.pname, O.cid, C.cname, qty, price, total  FROM tbOrder AS O JOIN tbCustomer AS C ON O.cid=C.id JOIN tbProduct AS P ON O.pid=P.pid WHERE CONCAT (orderid, odate, O.pid, P.pname, O.cid, C.cname, qty, price) LIKE '%" + txtSearch.Text + "%'", con);
